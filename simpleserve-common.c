...
#define SS_SAMBA_INCLUDE_BEGIN "# BEGIN SimpleServe managed Samba include"
#define SS_SAMBA_INCLUDE_END "# END SimpleServe managed Samba include"
...
{
    const char *override = getenv("SIMPLESERVE_SMB_CONF");
...
{
    const char *override = getenv("SIMPLESERVE_SAMBA");
...
static int ss_samba_account_name_valid(const char *name)
{
    size_t length;

    if (!name || (length = strlen(name)) == 0 || length > 255)
        return 0;
    for (size_t index = 0; index < length; index++) {
        unsigned char character = (unsigned char)name[index];
...
static int ss_samba_share_name_valid(const char *name)
{
    return ss_valid_name(name) && strcasecmp(name, "global") != 0 &&
           strcasecmp(name, "homes") != 0 &&
           strcasecmp(name, "printers") != 0;
}

static int ss_resolve_samba_owner(const SSLocalShare *share,
                                  char *user, size_t user_size,
                                  char *group, size_t group_size,
                                  char *error, size_t error_size)
{
    struct passwd account;
    struct passwd *account_result = NULL;
    struct group group_entry;
    struct group *group_result = NULL;
    char account_buffer[16384];
    char group_buffer[16384];
    int status;

    status = getpwuid_r(share->owner_uid, &account, account_buffer,
                        sizeof(account_buffer), &account_result);
    if (status != 0 || !account_result ||
        !ss_samba_account_name_valid(account_result->pw_name) ||
        !ss_copy_string(user, user_size, account_result->pw_name)) {
        ss_error(error, error_size,
                 "cannot resolve a Samba account for share %s owner uid %llu",
                 share->name, (unsigned long long)share->owner_uid);
        return 0;
    }
    status = getgrgid_r(share->owner_gid, &group_entry, group_buffer,
                        sizeof(group_buffer), &group_result);
    if (status != 0 || !group_result ||
        !ss_samba_account_name_valid(group_result->gr_name) ||
        !ss_copy_string(group, group_size, group_result->gr_name)) {
        ss_error(error, error_size,
                 "cannot resolve a Samba group for share %s owner gid %llu",
                 share->name, (unsigned long long)share->owner_gid);
        return 0;
    }
    return 1;
}
...
{
    if (!config || !output) {
        ss_error(error, error_size, "invalid Samba generation request");
        return 0;
    }
    if (!ss_buffer_append(
            output,
    ...
    for (size_t index = 0; index < config->share_count; index++) {
        ...
        if (!share->active)
            continue;
        if (!ss_samba_share_name_valid(share->name)) {
            ss_error(error, error_size,
                     "share %s is not a usable Samba share name", share->name);
            return 0;
        }
        for (size_t previous = 0; previous < index; previous++) {
            const SSLocalShare *other = &config->shares[previous];

            if (other->active && strcasecmp(other->name, share->name) == 0) {
                ss_error(error, error_size,
                         "Samba share names differ only by case: %s and %s",
                         other->name, share->name);
                return 0;
            }
        }
        if (!ss_valid_absolute_path(share->current_path)) {
            ss_error(error, error_size,
                     "share %s has an invalid active Samba path", share->name);
            return 0;
        }
        if (!ss_resolve_samba_owner(share, user, sizeof(user), group,
                                    sizeof(group), error, error_size))
            return 0;
        if (!ss_buffer_appendf(
                output,
                "\n[%s]\n"
                ...
                share->access == SS_ACCESS_READ_ONLY ? "yes" : "no",
                user, group))
            goto memory_error;
    }
    return 1;
...
memory_error:
    ss_error(error, error_size,
             "out of memory while generating Samba configuration");
    return 0;
}
                                     ...
                                     size_t error_size)
{
    ...
    if (!include_path || !output) {
        ss_error(error, error_size, "invalid Samba include request");
        return 0;
    }
    ss_buffer_init(&managed);
    if (include_path[0] &&
        (!ss_valid_absolute_path(include_path) ||
         !ss_buffer_appendf(&managed, "[global]\ninclude=%s\n",
                            include_path))) {
        if (!ss_valid_absolute_path(include_path))
            ss_error(error, error_size, "invalid Samba include path");
        else
            ss_error(error, error_size,
                     "out of memory while generating Samba include");
        ss_buffer_free(&managed);
        return 0;
    }
    ok = ss_replace_managed_block(
        existing, managed.data ? managed.data : "", SS_SAMBA_INCLUDE_BEGIN,
        SS_SAMBA_INCLUDE_END, "/etc/samba/smb.conf", output, error,
        error_size);
    ss_buffer_free(&managed);
    return ok;
}
